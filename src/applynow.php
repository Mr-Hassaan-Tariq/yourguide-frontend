

<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">

    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <link rel="icon" href=images/logo.jpg"
          type="image/x-icon">
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css"
          integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh"
          crossorigin="anonymous">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Ubuntu:wght@300&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.15.4/css/all.css"
          integrity="sha384-DyZ88mC6Up2uqS4h/KRgHuoeGwBcD4Ng9SiP4dIRy0EXTlnuz47vAwmeGwVChigm" crossorigin="anonymous">

    <link rel="canonical" href="https://getbootstrap.com/docs/3.4/examples/navbar-fixed-top/">
    <script src="https://code.jquery.com/jquery-3.2.1.slim.min.js"
            integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN"
            crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.9/umd/popper.min.js"
            integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q"
            crossorigin="anonymous"></script>
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js"
            integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl"
            crossorigin="anonymous"></script>
    <meta content="" name="description">
    <meta content="" name="keywords">

    <!-- Favicons -->
    <link href="assets/img/favicon.png" rel="icon">
    <link href="assets/img/apple-touch-icon.png" rel="apple-touch-icon">

    <!-- Google Fonts -->
    <link href="https://fonts.googleapis.com/css?family=Open+Sans:300,300i,400,400i,600,600i,700,700i|Raleway:300,300i,400,400i,500,500i,600,600i,700,700i|Poppins:300,300i,400,400i,500,500i,600,600i,700,700i"
          rel="stylesheet">

    <!-- Vendor CSS Files -->
    <link href="assets/vendor/aos/aos.css" rel="stylesheet">
    <link href="assets/vendor/bootstrap/css/bootstrap.min.css" rel="stylesheet">
    <link href="assets/vendor/bootstrap-icons/bootstrap-icons.css" rel="stylesheet">
    <link href="assets/vendor/boxicons/css/boxicons.min.css" rel="stylesheet">
    <link href="assets/vendor/glightbox/css/glightbox.min.css" rel="stylesheet">
    <link href="assets/vendor/swiper/swiper-bundle.min.css" rel="stylesheet">


    <!-- Template Main CSS File -->
    <link href="assets/css/style.css" rel="stylesheet">


    <link rel="stylesheet" href="css%20files/applynow.css">
    <script src="js%20files/applynow.js"></script>
    <!--                    <script src="index.js"></script>-->

    <title>
        APPLY NOW
    </title>

</head>
<body>


<i class="bi bi-list mobile-nav-toggle d-xl-none"></i>
<!-- ======= Header ======= -->
<header id="header" class="d-flex flex-column justify-content-center">

    <nav id="navbar" class="navbar nav-menu">
        <ul>
            <li><a href="routes.php" class="nav-link scrollto active"><i class="bx bx-home"></i> <span>Home</span></a>
            </li>
        </ul>
    </nav><!-- .nav-menu -->

</header><!-- End Header -->

<div class="popup" id="popup">
    <div class="overlay"></div>
    <div class="content">
        <!--                    <div class="close-btn" onclick="closePopup()">&times;</div>-->
        <div class="row">

            <div  class="col-lg-12 col-md-9  col-sm-12" style="padding-top: 15%;">
<!--                <img width="200" id="cImage" height="200" src="https://cdn.dribbble.com/users/2469324/screenshots/6538803/comp_3.gif">-->
                <h2 id="errorHead">fields are required</h2>
                <p id="errorMessage">
                    All fields are required. fill all fields. Or contact to the company for this issue. email : writetoyourguide@gmail.com
                </p>
                <button style="background-color: darkgreen; width: 130px; margin-top: 5%;" class="btn btn-success radiusChange backgroundChange" onclick="closePopup()"> Close</button>
            </div>
        </div>
    </div>
</div>

<div class="row">
    <div class="col-lg-12" style="padding-top:5%;">
        <h1 class="centerAlign">Get in touch</h1>
        <p class="centerAlign" style="padding-left:20%; padding-right:20%;">
            If you're looking to collaborate with us, got something to say, or \
            just want to tell us about a nifty place in town, all you gotta do is hit us up! We'd love to hear from you.
        </p>
        <div class="row" style="padding-left:10%; padding-right:10%;">
            <div class="col-lg-4 centerAlign " style="padding: 5%;">
                <h1 class="changeColor" style="font-size: 70px;"><i class="fab fa-instagram"></i></h1>
                <h2 class="changeColor">Instagram</h2>
                <h4><a class="changeColor" href="https://www.instagram.com/lahore.guide/ ">yourguide.city</a></h4>
            </div>
            <div class="col-lg-4 " style="text-align: center; padding: 5%;">
                <h1 class="changeColor" style="font-size: 70px;"><i class="fas fa-envelope"></i></h1>
                <h2 class="changeColor">Email</h2>
                <h4><a class="changeColor" href="# ">writetoyourguide@gmail.com</a></h4>
            </div>
            <div class="col-lg-4 centerAlign" style="padding: 5%;">
                <h1 class="changeColor" style="font-size: 70px;"><i class="fab fa-youtube"></i></h1>
                <h2 class="changeColor">Youtube</h2>
                <h4><a class="changeColor" href="https://www.youtube.com/channel/UCqyh3o8AwgnXByDNuzyG3gQ">yourguide.city</a>
                </h4>
            </div>
        </div>
    </div>

</div>
<div class="row " style="align-items: center; padding-left: 20%;">

    <div  class="col-lg-7 formstyle">
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" role="form" enctype="multipart/form-data" class="php-email-form">
            <div class="form-inline">
                <input style="width:200px;" name="firstName" class="form-control" type="text" placeholder="first name">
                <input style="width:200px;" name="lName" class="form-control" type="text" placeholder="last name">
            </div>
            <input class="form-control" name="number" type="text" placeholder="number">
            <input class="form-control"  name="email" type="text" placeholder="email">
            <label for="file">Resume</label>
            <input class="form-control" type="file" id="file" name="file"  data-msg-placeholder="Select {files} for upload..."  placeholder="your resume">

            <textarea class="form-control" placeholder="Cover Latter" onkeyup="textEntered()" name="cover" id="detailField" rows="3"></textarea>


            <button class="btn btn-white btn-lg btnClr "   name="submit"  id="btnSubmit">
                Submit
            </button>
        </form>

    </div>
</div>

<footer>
    <div class="row" style="padding-left: 5%;padding-right: 5%;">

        <div class="col-lg-7">

            <div class="row">
                <div class="col-lg-3 ">
                    <img src="images/logo.jpg" width="200" height="200">
                </div>
                <div class="col-lg-7 topPadding">
                    <p>
                        Looking To Explore Your City? OR Looking to get the best out of your holiday?
                        We’ve Got Your Back! Let us help by getting you a great guide; someone who will help you
                        navigate local language and culture barriers,
                        show you hidden gems, best eateries, trendy stores, cool events, getaways and tell you stories
                        that only a local would know.
                    </p>
                    <hr>
                </div>
            </div>
        </div>
        <div class="col-lg-5 alignLeft topPadding">
            <h4>Our social media handles</h4>
            <div class="form-inline">

                <a class=" " href="https://www.linkedin.com/company/yourultimateguide/" target="_blank"><img width="42"
                                                                                                             height="42"
                                                                                                             src="data:image/png;base64,"></a>
                <a style="" href="https://www.facebook.com/yourguide.city" target="_blank"><img width="42" height="42"
                                                                                                src="data:image/png;base64,"></a>
                <a href="https://www.youtube.com/channel/UCqyh3o8AwgnXByDNuzyG3gQ" target="_blank"><img width="42"
                                                                                                        height="42"
                                                                                                        src="https://w7.pngwing.com/pngs/936/468/png-transparent-youtube-logo-youtube-logo-computer-icons-subscribe-angle-rectangle-airplane.png"></a>
                <a href="https://www.instagram.com/lahore.guide/" target="_blank"><img width="42" height="42"
                                                                                       src="https://png.pngtree.com/png-clipart/20190516/original/pngtree-instagram-social-media-icon-design-template-vector-png-image_3654775.jpg"></a>
                <a href="https://twitter.com/LahoreGuide" target="_blank"><img width="42" height="42"
                                                                               src="https://cliply.co/wp-content/uploads/2019/07/371907030_TWITTER_ICON_1080.png"></a>
            </div>
        </div>
    </div>
</footer>


<script src="assets/js/main.js"></script>
<script src="js%20files/index.js"></script>
</body>
</html>



<?php

$fName = $lName = $number = $email = $cover = "";

if (isset($_POST['submit'])) {
    $connection = null;
    $fName = $_POST['firstName'];
    $lName = $_POST['lName'];
    $number = $_POST['number'];
    $email = $_POST['email'];
    $cover = $_POST['cover'];
    $flag = true;
    if (empty($fName) || empty($lName) || empty($number) || empty($email)){
        echo' <script type="text/javascript">
        
                popup();
              </script>';
        $flag = false;
    }
    if ($flag) {

        connectToDatabase();
        if(!userPresent($email)) {


                    addCandidate($fName, $lName, $email, $number, $cover, $GLOBALS['connection']);




                    $currentDirectory = getcwd();
                    $uploadDirectory = "/resumes/";

                    $errors = []; // Store errors here

                    $fileExtensionsAllowed = ['pdf', 'docs']; // These will be the only file extensions allowed

                    $fileName = $_FILES['file']['name'];
                    $fileSize = $_FILES['file']['size'];
                    $fileTmpName = $_FILES['file']['tmp_name'];
                    $fileType = $_FILES['file']['type'];
                    $array = explode('.', $fileName);
                    $fileExtension = strtolower(end($array));

                    $newFileName = $fName."_".$lName.date("h_i_s");
                    $uploadPath = $currentDirectory . $uploadDirectory . $newFileName . "." . $fileExtension;

                    if (!in_array($fileExtension, $fileExtensionsAllowed)) {
                        $error = "This file extension is not allowed. Please upload a JPEG or PNG file";
                        echo '<script>alert("This file extension is not allowed. Please upload a pdf or docs file")</script>';
                    }

                    if ($fileSize > 4000000) {
                        $errors[] = "File exceeds maximum size (4MB)";
                    }

                    if (empty($errors)) {
                        $didUpload = move_uploaded_file($fileTmpName, $uploadPath);

                        if ($didUpload) {


                            ?>
                                <script type="text/javascript" src="js%20files/index.js">
                                                popup();

                                </script>
                            <?php

                        } else {
                            echo '<script>alert("An error occurred. Please contact the administrator.")</script>';

                        }
                    } else {
                        foreach ($errors as $error) {
                            echo '<script>alert($error . "These are the errors" . "\n")</script>';

                        }
                    }

                }

        else{


            ?>
            <script>
                                document.getElementById("errorHead").innerHTML = "you have already applied for this post.";
                                document.getElementById("errorMessage").innerHTML = "Your email that you entered is already present in our server.try with another email. Or contact to the company for this issue. email : writetoyourguide@gmail.com ";
                                popup();

                </script>
            <?php
        }

        $GLOBALS['connection']->close();

    }


}

function connectToDatabase()
{

//            $GLOBALS['connection'] = mysqli_connect("localhost", "youryyuu_root", "cNzDhw8@", "youryyuu_candidate");
    $GLOBALS['connection'] = mysqli_connect("localhost", "root", "root", "candidate");
    if ($GLOBALS['connection'] == null){
        echo '<script type="text/javascript">
                        document.getElementById("errorMessage").innerHTML = "contact to the company for this issue. email : writetoyourguide@gmail.com ";
                        
                    
                        document.getElementById("errorHead").innerHTML = "some problem is occur ";

                        document.getElementById("popup").style.display = "block";
                        document.getElementById("popup").classList.toggle("active");
                   
                </script>';
    }
    else {
        makeTable();
    }

}
function makeTable()
{
    $sql = "CREATE TABLE IF NOT EXISTS users (
                                                id INT(6) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
                                                firstname VARCHAR(30) NOT NULL,
                                                lastname VARCHAR(30) NOT NULL,
                                                email VARCHAR(50) not null,
                                                number varchar(50) not null,
                                                cover varchar(500),
                                                date varchar(15) not null
                                                )";

    if ($GLOBALS['connection']->query($sql) === TRUE) {
        //                        echo "Table users created successfully";
    } else {
//                echo "Error creating table: " . $GLOBALS['connection']->error;
        ?> '<script type="text/javascript">
                        document.getElementById("errorMessage").innerHTML = "contact to the company for this issue. email : writetoyourguide@gmail.com ";
                        
                    
                        document.getElementById("errorHead").innerHTML = "some problem is counter";
                       
                        document.getElementById("popup").style.display = "block";
                        document.getElementById("popup").classList.toggle("active");
                   
                </script>'<?php

    }
}

function userPresent($chk){
    $sql = "SELECT * FROM users WHERE email='$chk'";
    $result = $GLOBALS['connection']->query($sql);

    if ($result->num_rows > 0) {
        return true;

    } else {
        return false;
    }
}
function addCandidate($first, $second, $email,$number, $cover, $connect){
    $date = date("h:i:sa");
    $sql = "INSERT INTO users(firstname, lastname, email,number,cover,date)
                                    VALUES ('$first','$second' ,'$email','$number','$cover','$date')";

    if ($connect->query($sql) === TRUE) {

        echo '<script type="text/javascript">
                        document.getElementById("errorMessage").innerHTML = "Thanks for your response. We will connect you";
                        
                    
                        document.getElementById("errorHead").innerHTML = "Your request has been submitted";
//                        document.getElementById("cImage").src = window.URL.createObjectURL("https://cdn.dribbble.com/users/1238709/screenshots/4069900/success_celebration_800x600.gif");
//                        https://cdn.dribbble.com/users/1238709/screenshots/4069900/success_celebration_800x600.gif
//                         document.getElementById("cImage").innerHTML = image:url("https://cdn.dribbble.com/users/1238709/screenshots/4069900/success_celebration_800x600.gif");
                        
                        document.getElementById("popup").style.display = "block";
                        document.getElementById("popup").classList.toggle("active");
                   
                </script>';
    } else {
        echo '<script type="text/javascript">
                        document.getElementById("errorMessage").innerHTML = "contact to the company for this issue. email : writetoyourguide@gmail.com ";
                        
                    
                        document.getElementById("errorHead").innerHTML = "some problem is counter";
//                        https://cdn.dribbble.com/users/1238709/screenshots/4069900/success_celebration_800x600.gif
//                         document.getElementById("cImage").innerHTML = image:url("https://cdn.dribbble.com/users/1238709/screenshots/4069900/success_celebration_800x600.gif");
                        
                        document.getElementById("popup").style.display = "block";
                        document.getElementById("popup").classList.toggle("active");
                   
                </script>';
    }
}


?>